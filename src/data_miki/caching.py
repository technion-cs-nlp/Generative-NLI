...
def embed_and_cache(cache_tag: str, dataset, set_type, encoder, output_dir, balance=True, collate_fn=None,
    ...
    if balance:
        ds_seed = 42
        positive_dataset = dataset.filter(lambda x: x['probing_label'] == 1)
        negative_dataset = dataset.filter(lambda x: x['probing_label'] == 0) \
                                  .shuffle(seed=ds_seed).select(np.arange(positive_dataset.num_rows))
        # num_samples = 2 * num_positive_samples
        # sampler = ImbalancedDatasetSampler(dataset, num_samples=num_samples,
        #                                    callback_get_label=lambda d, i: all_labels[i].item())
        dataset = concatenate_datasets([positive_dataset, negative_dataset]).shuffle(seed=ds_seed)
